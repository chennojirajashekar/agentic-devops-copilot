...
class AgenticDevOpsOrchestrator:
    ...
    def run(self, raw_alert: dict = None) -> WorkflowResult:
        ...
        rollout = self.change_agent.orchestrate_rollout(
            incident, fix, review, start
        )
        logger.info(
            " -> %s | MTTR=%.2fm",
            rollout.status,
            rollout.actual_mttr_minutes
        )
        ...
        summary = (
            ...
            f"Review : {review.recommendation} "
            f"(risk={review.risk_score}/100)\n"
